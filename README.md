...
- Майстер запитів: симулятор створення корисного промпта без передачі даних у зовнішній AI-сервіс.
...
- PWA-заготовка: `manifest.webmanifest`, `sw.js` і SVG-іконки для офлайн-кешування та встановлення.
- Контент винесено в `data/content.json`.
- Onboarding-блок пояснює, як відкриваються рівні та де зберігається прогрес.
...
Прогрес зберігається в `localStorage` конкретного браузера. Це просте рішення без реєстрації та бази даних. Якщо користувач очистить дані сайту або відкриє інший браузер, прогрес почнеться спочатку.

## Про реальну AI-інтеграцію

У браузерній статичній версії не можна безпечно зберігати секретний API-ключ. Для реального ChatGPT/OpenAI demo потрібен невеликий сервер або serverless-функція, яка:
